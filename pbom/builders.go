...
// nobody ran, inside the document the flag exists to produce. With nil the
// document records the inventory and claims nothing.
//
// Two runs may not claim them. noControls (--no-controls) evaluated nothing
// at all. And a platform-mode run whose resolved policies do not enable an
// image control evaluated it for no policy, which is the same absence of a
// verdict; the findings it does have come from the policy runs' union, so the
// flags it CAN claim are the policies' own.
//
...
//
// It lives here rather than beside one writer because both the PBOM
// artifacts and the platform push's bill of materials are enriched from it,
// and the two must never disagree about which verdict this run may claim.
func ImageComplianceFor(result *control.AnalysisResult, noControls bool, platform *PlatformSummary) *ImageComplianceData {
	if result == nil || noControls {
		return nil
	}
	data := BuildImageComplianceData(result)
	if platform == nil {
		return data
	}
