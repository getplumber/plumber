...
func TestBuildCSV_ShapeColumnsAndOrdering(t *testing.T) {
	...
	result := &control.AnalysisResult{
		...
		Findings: []opaengine.Finding{
			{Code: "ISSUE-701", Severity: "high", Message: "unpinned action", Job: "ci/build", File: "./.github/workflows/ci.yml", Line: 28, URL: "https://example.com/blob/ci.yml#L28"},
		},
	}

	records := buildCSV(entries, result)

	wantHeader := []string{"code", "controlName", "status", "severity", "message", "context", "file", "line", "url", "docUrl"}
	...
	// Non-failing controls (passed, skipped) come first, in catalog order.
	if records[1][1] != "cleanControl" || records[1][2] != "passed" {
		t.Errorf("row1 = %v, want cleanControl/passed", records[1][:3])
	}
	if records[1][0] != "" {
		t.Errorf("passed row code = %q, want empty", records[1][0])
	}
	if records[2][1] != "disabledControl" || records[2][2] != "skipped" {
		t.Errorf("row2 = %v, want disabledControl/skipped", records[2][:3])
	}
	if records[2][4] != "disabled in configuration" {
		t.Errorf("skipped row message = %q, want the skip reason", records[2][4])
	}
	...
	want := map[int]string{
		0: "ISSUE-701",
		1: "actionsMustBePinnedByCommitSha",
		2: "failed",
		3: "high",
		4: "unpinned action",
		5: "ci/build",
		...
		7: "28",
		8: "https://example.com/blob/ci.yml#L28",
		9: "https://getplumber.io/docs/cli/issues/ISSUE-701",
	}
	for i, w := range want {
		if failRow[i] != w {
...
func TestBuildCSV_CleanRunListsEveryControl(t *testing.T) {
	...
	for _, r := range records[1:] {
		if r[2] != "passed" {
			t.Errorf("clean control status = %q, want passed", r[2])
		}
	}
}
...
func TestBuildCSV_NonFailingBeforeFailing(t *testing.T) {
	...
	records := buildCSV(entries, result)
	if records[1][2] != "passed" {
		t.Errorf("first data row status = %q, want passed (non-failing at top)", records[1][2])
	}
	last := records[len(records)-1]
	if last[2] != "failed" {
		t.Errorf("last data row status = %q, want failed (problems at the bottom)", last[2])
	}
}
...
func TestBuildCSV_ErrorStatusOnDegradedRun(t *testing.T) {
	...
	records := buildCSV(entries, result)
	if records[1][2] != "error" {
		t.Errorf("degraded clean control status = %q, want error", records[1][2])
	}
	if records[1][4] == "" {
...
func TestBuildCSV_LineZeroIsEmptyString(t *testing.T) {
	...
	records := buildCSV(entries, result)
	if records[1][7] != "" { // line column
		t.Errorf("line = %q, want empty string for zero-value Line", records[1][7])
	}
}
