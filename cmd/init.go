...
// annotateGitleaksDisabled marks any pipelineMustNotLeakSecretsInConfig
// block the wizard emitted with a note that secret scanning is temporarily
// disabled, so a generated config does not imply the control is protecting
// the user when it silently abstains. It deliberately does NOT advertise the
// gitleaksPath / gitleaksConfigPath overrides: gitleaksPath was the vector
// behind the resolved RCE, and a generated config must never steer a user
// toward setting it while the integration is reworked. yaml.v2 emits a stable
// 2-space indentation for nested keys, so a literal match on the canonical
// "pipelineMustNot…:\n      enabled: true\n" pair is stable across runs.
func annotateGitleaksDisabled(b []byte) []byte {
	const (
		needle = "pipelineMustNotLeakSecretsInConfig:\n      enabled: true\n"
		note   = "      # Secret scanning is temporarily disabled pending a security\n" +
			"      # fix; this control abstains and runs no scanner regardless of\n" +
			"      # the value above.\n"
	)
	return bytes.ReplaceAll(b, []byte(needle), []byte(needle+note))
}
...
func marshalAndWriteConfig(cfg *configuration.PlumberConfig, path, generatedBy string) error {
	...
	out = annotateGitleaksDisabled(out)
