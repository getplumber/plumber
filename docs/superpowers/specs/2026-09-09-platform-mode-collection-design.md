...
6. Docs: `README` platform section note (platform mode now evaluates ISSUE-506 and ISSUE-601 from the snapshot; the four degraded identifiers recognized; the served raw config and the commit it is used at). No `CHANGELOG.md` entry: the file is generated by semantic-release from the conventional commit subjects, so a hand-written section would be duplicated or misplaced at tag time.
